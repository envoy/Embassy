...
struct HeapSort {
    ...
    static func heapPush<T>(_ heap: inout [T], item: T, isOrderredBefore: (T, T) -> Bool) {
        heap.append(item)
    ...
    ///  - Parameter heap: the heap queue array, should already be in heap order
    ///  - Parameter item: the new item to be appended into the heap queue
    static func heapPush<T: Comparable>(_ heap: inout [T], item: T) {
    ...
    static func heapPop<T>(_ heap: inout [T], isOrderredBefore: (T, T) -> Bool) -> T {
    ...
    private static func shiftUp<T>(_ heap: inout [T], pos: Array<T>.Index, isOrderredBefore: (T, T) -> Bool) {
        var pos = pos
