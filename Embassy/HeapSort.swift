...
struct HeapSort {
    ...
    static func heapPush<T: Comparable>(inout heap: Array<T>, item: T) {
        heap.append(item)
        shiftDown(&heap, startPos: 0, pos: heap.count - 1)
    }
    ...
    private static func shiftDown<T: Comparable>(inout heap: Array<T>, startPos: Array<T>.Index, pos: Array<T>.Index) {
        var pos = pos
    ...
    private static func shiftUp<T: Comparable>(inout heap: Array<T>, pos: Array<T>.Index) {
        var pos = pos
        ...
        heap[pos] = newItem
        shiftDown(&heap, startPos: startPos, pos: pos)
    }
}
